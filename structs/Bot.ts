	...
	private async onInteractionCreate() {
		this.client.on(
			...
			async (interaction: Interaction): Promise<any> => {
				...
				const command = this.slashCommandsMap.get(
				...
				const now = Date.now();
				const timestamps: any = this.cooldowns.get(
					interaction.commandName
				);
				...
				try {
					const permissionsCheck: PermissionResult =
						await checkPermissions(command, interaction);
